...
export default function SearchCandidates() {
  ...
  const filtered = useMemo(() => {
    ...
    if (experienceFrom) data = data.filter((c) => c.experienceYears >= Number(experienceFrom));
    if (experienceTo) data = data.filter((c) => c.experienceYears <= Number(experienceTo));

    if (city.trim())
      data = data.filter((c) => c.location.toLowerCase().includes(city.toLowerCase()));
    ...
    data = data.filter((c) => c.lastActiveDays >= minD && c.lastActiveDays <= maxD);
        ...
        {isMdUp ? (
          <Box sx={{ display: "flex", alignItems: "stretch" }}>
            ...
            >
              ...
              <Paper sx={{ p: 2, borderRadius: 1, boxShadow: "none" }}>
            ...
            <Box
              ...
              aria-hidden
            >
              <Box sx={{ width: 2, height: 40, background: "rgba(15,23,42,0.08)", borderRadius: 1 }} />
            </Box>
          </Box>
        ) : (
          <Box sx={{ width: { xs: "100%" } }} />
